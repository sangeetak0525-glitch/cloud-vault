...
def upload_file(
    ...
    # Determine the target owner for this upload.
    target_owner_id = user.id
    target_user = user
    if owner_id is not None:
        # Only admins may set a different owner_id
        if user.role != "admin":
    ...
    # Save to uploads/<owner_name>_<owner_id>/filename
    owner_folder = make_owner_folder(target_user)
    user_dir = os.path.join(UPLOAD_DIR, owner_folder)
...
# ─── DOWNLOAD: authenticated file access ────────────────────────────────────
...
def admin_all_files(token: str = Query(...), db: Session = Depends(get_db)):
    ...
    return [
        {"id": f.id, "name": f.name, "size": f.size, "owner_id": f.owner_id,
         "owner_name": owner_name, "shared": f.shared, "starred": f.starred, "trash": f.trash, "created_at": str(f.created_at)[:10]}
        for f, owner_name in files
    ]


# ─── SHARE: create public share token ───────────────────────────────────────
...
# ─── DOWNLOAD: access shared file without auth ─────────────────────────────
